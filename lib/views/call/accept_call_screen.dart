...
class _AcceptCallScreenState extends State<AcceptCallScreen> {
      ...
      timer = Timer.periodic(Duration(seconds: 5), (timer) async {
        ...
        if (!isStart) {
          ...
          await _callController.getAgoraResourceId(
              widget.callChannel!, global.myAudioRecordingUid);
          await startRecord();
        }
      });
    } else {
  ...
  Future startRecord() async {
    await callController.agoraStartRecording(
        widget.callChannel!, widget.token!, global.myAudioRecordingUid);
  }

  Future stopRecord() async {
    await callController.agoraStopRecording(
        widget.callId!, widget.callChannel!, global.myAudioRecordingUid);
  }
